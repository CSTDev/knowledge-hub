...
Set the following environment variables:<br/>
  REACT_APP_VERSION - the current version (defaults to 0.0.1)<br/>
  REACT_APP_VERSION_COLOUR - the colour for the version bar (defaults to #58af58 which is green)<br/>
  REACT_APP_MAP_PROVIDER - where to get the map tiles from e.g. http://{s}.tile.osm.org/{z}/{x}/{y}.png (defaults to this OSM URL)<br/>
  REACT_APP_API_URL - URL of the knowledge API (defaults to what's in .env files)<br/>
